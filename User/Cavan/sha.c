...
static u8 *cavan_sha_invert8(u8 *dest, const u8 *src, size_t length)
{
	const u8 *tail = src + length;
...
static u32 *cavan_sha_swap32(u32 *dest, const u32 *src, size_t length)
{
	u32 *dest_end = dest + length;

	while (dest < dest_end) {
		u32 value = *src++;
		*dest++ = SHA_SWAP32(value);
	}
...
void cavan_sha_finish(struct cavan_sha_context *context, u8 *digest)
{
	...
	if (context->flags & SHA_FLAG_SWAP) {
		cavan_sha_invert8(context->buff + sizeof(context->buff) - sizeof(bits), (void *) &bits, sizeof(bits));
	} else {
	...
	if (context->flags & SHA_FLAG_SWAP) {
		cavan_sha_swap32((u32 *) digest, context->digest, context->digest_size >> 2);
	} else {
...
int cavan_file_sha_mmap(struct cavan_sha_context *context, const char *pathname, u8 *digest)
{
	...
	ret = cavan_sha_check(context, addr, size, digest);
...
int cavan_file_sha(struct cavan_sha_context *context, const char *pathname, u8 *digest)
{
	...
	ret = cavan_file_sha_mmap(context, pathname, digest);
	if (ret >= 0) {
		return ret;
	}
...
void cavan_sha_show_digest(struct cavan_sha_context *context, void *digest)
{
	...
	length = cavan_dump_hex_bytes(digest, context->digest_size, text, sizeof(text)) - text;
	print_ntext(text, length);
	putchar('\n');
}
#endif
...
void cavan_md5_check(const void *buff, size_t length, void *digest)
{
	...
	cavan_sha_check(&context, buff, length, digest);
}
...
{
	struct cavan_sha_context context;
	cavan_sha1_init_context(&context);
	cavan_sha_check(&context, buff, length, digest);
}
