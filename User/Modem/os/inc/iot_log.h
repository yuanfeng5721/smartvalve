...
#define LOG_GEN(file, name, line, level, format, args...) \
		{\
			extern LOG_LEVEL g_log_print_level;\
			if(level<=g_log_print_level)\
			{\
				printf("%s|%s(%d): " format, file, name, line, ##args);\
			}\
		}\

#define Log_d(fmt, args...) LOG_GEN(__FILE__, __FUNCTION__, __LINE__, eLOG_DEBUG, fmt, ##args)
#define Log_i(fmt, args...) LOG_GEN(__FILE__, __FUNCTION__, __LINE__, eLOG_INFO, fmt, ##args)
#define Log_w(fmt, args...) LOG_GEN(__FILE__, __FUNCTION__, __LINE__, eLOG_WARN, fmt, ##args)
#define Log_e(fmt, args...) LOG_GEN(__FILE__, __FUNCTION__, __LINE__, eLOG_ERROR, fmt, ##args)
#endif
