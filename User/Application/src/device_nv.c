...
const uint16_t press_back_default_value[PB_DEFAULT_NUM]=
...
const uint16_t q_default_value[Q_DEFAULT_NUM]=
...
const uint8_t angle_default_value[ANGLE_DEFAULT_NUM] =
...
static uint8_t  NVVERSION	= 2;  //default nv version, if need update default nv, please  NV_VERSION + 1
static uint8_t  update_freq = 20;
static uint32_t wakeup_count = 0;
static uint8_t  F_value = 1;
static uint8_t  warring_flag = false;
static uint8_t  pre_set_angle = 0;
static uint16_t f_press_max = 100;
static uint16_t f_press_min = 0;
static uint16_t b_press_max = 100;
static uint16_t b_press_min = 0;
static uint8_t  encoder_count = 100;
static uint16_t moto_timer_count = 382;

const ef_env default_env_set[DEFAULT_NV_ITEMS] = {
		MAKE_NV_ITEM_STR("TEST",				"12345678"),
		...
		MAKE_NV_ITEM_STR(MOTO_POSITION, "0"),
...
//		MAKE_NV_ITEM_INT(MOTO_POSITION_MAX,		11500),
		MAKE_NV_ITEM_INT(NV_WARRING_FLAG,		&warring_flag, sizeof(warring_flag)),
//		MAKE_NV_ITEM_INT(NV_ONE_CIRCLE_PLUS,	11200),
		...
		MAKE_NV_ITEM_ARRAY(PB_DEFAULT_KEY, press_back_default_value, sizeof(press_back_default_value)),
		MAKE_NV_ITEM_ARRAY(Q_DEFAULT_KEY, q_default_value, sizeof(q_default_value)),
		MAKE_NV_ITEM_ARRAY(ZETA_KEY, zeta_value, sizeof(zeta_value)),
		MAKE_NV_ITEM_ARRAY(ANGLE_DEFAULT_KEY, angle_default_value, sizeof(angle_default_value)),
#endif
};
...
void print_array_nv(const char *key, void *value, size_t len)
{
	LOG_RAW("%s: [", key);
	for(int i=0; i<len; i++) {
		LOG_RAW("%d ",((uint8_t *)value)[i]);
	}
	LOG_RAW("]\r\n");
}
